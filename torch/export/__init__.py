...
class Constraint(_ConstraintTarget, metaclass=_ConstraintFactory):
    ...
    .. warning::
        Do not construct `Constraint` directly, use :func:`dynamic_dim` instead.
...
def export(
    ...
    **Acceptable input/output types**
    ...
    - (Nested) Data structures comprising of `dict`, `list`, `tuple`, `namedtuple` and
      `OrderedDict` containing primitive types.
    ...
    **What's specialized in the program?**
    ...
    :func:`export` specializes the traced program based on the values of
    ...
    would yield an :class:`ExportedProgram` containing following graph::
    ...
    specified as dynamic via :func:`dynamic_dim` API, for example::
    ...
    Would produce an :class:`ExportedProgram` containing following graph::
    ...
    example inputs has the static shape of `(10, 2)`. :func:`export` specializes
    ...
    :func:`dynamic_dim` to make a dimension of input tensor to be dynamic
    and rewrite the source code using control flow operations like
    :func:`torch.ops.higher_order.cond`.
    ...
    **How to express Dynamism**
    ...
    Because static shape use cases are more dominant, :func:`export` chooses to
    ...
    :func:`dynamic_dim` to give a hint to :func:`export` about dynamism
    ...
    **ExportedProgram Invariants**
    ...
    The returned :class:`ExportedProgram` maintains the following invariants:
    ...
    - It contains a `state_dict` that stores the :class:`torch.nn.Parameters`
      and registered buffers involved in computation of the original program.
    - It includes an :class:`fx.GraphModule` that represents the computation of
      the original program. The :class:`fx.GraphModule`:
